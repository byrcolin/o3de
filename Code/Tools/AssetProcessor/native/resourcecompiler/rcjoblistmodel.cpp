...
namespace AssetProcessor
{
    ...
    {
        return static_cast<unsigned int>(m_finishedJobsNotInCatalog.count());
    }
    ...
    void RCJobListModel::PerformHeuristicSearch(QString searchTerm, QString platform, QSet<QueueElementID>& found, AssetProcessor::JobIdEscalationList& escalationList, bool isStatusRequest, int searchRules)
    {
        ...
        int dotIndex = static_cast<int>(searchTerm.lastIndexOf('.'));
        while (dotIndex != -1)
        {
            ...
            for (const auto& rcJob : m_jobs)
            {
                ...
                QString input = rcJob->GetJobEntry().m_sourceAssetReference.RelativePath().c_str();
                dotIndex = static_cast<int>(input.lastIndexOf('.'));
                if (dotIndex != -1)
                {
            ...
            dotIndex = static_cast<int>(searchTermWithNoExtension.lastIndexOf('.'));
        }
        ...
        int slashIndex = static_cast<int>(searchTerm.lastIndexOf('/'));
        int dashIndex = static_cast<int>(searchTerm.lastIndexOf('_'));
